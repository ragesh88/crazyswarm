...
def traj_rad_process(queue, b_box, Rob_active_lab_mat,
    ...
    # put the trajectories in to the queue
    queue.put(fail_trjs)
...
def cmpt_hover(crazyflies, timeHelper, Robot_state, duration):
    start_time = timeHelper.time()
    rob_lab_mat = sorted(Robot_state.keys())
    zero = np.zeros(3)
    while not timeHelper.isShutdown():
